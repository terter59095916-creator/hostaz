...
const badGuardStart = "(function(){var allowed=[\x27ureyimsen.com\x27,\x27[www.ureyimsen.com]";
...
files.forEach(f => {
  ...
  if (c.startsWith(badGuardStart)) {
    const endMarker = "})();\r\n";
    const endIdx = c.indexOf(endMarker) + endMarker.length;
    c = goodGuard + c.slice(endIdx);
    fs.writeFileSync(f, c, "utf8");
    console.log(f + ": DUZELDILDI");
  } else {
    console.log(f + ": KOHNE QORUMA TAPILMADI (baxilmali)");
  }
});
